...
function ProjectCard({ project, onUpdate, onSave, onDelete, expanded, onToggle }) {
  ...
  const [localCollabs, setLocalCollabs] = useState(project.collaborators || "");
  const [localOwner, setLocalOwner] = useState(project.owner || "");
  const [localNotes, setLocalNotes] = useState(project.notes || "");
  const [localBounce, setLocalBounce] = useState(project.bounce_link || "");
  const [localAssets, setLocalAssets] = useState(project.assets_link || "");

  useEffect(() => {
    setLocalTitle(project.title || "");
    setLocalCollabs(project.collaborators || "");
    setLocalOwner(project.owner || "");
    setLocalNotes(project.notes || "");
    setLocalBounce(project.bounce_link || "");
    setLocalAssets(project.assets_link || "");
  }, [project.id]);

  const debouncedTitle = useDebounce(localTitle, 800);
  const debouncedCollabs = useDebounce(localCollabs, 800);
  const debouncedOwner = useDebounce(localOwner, 800);
  const debouncedNotes = useDebounce(localNotes, 800);
  const debouncedBounce = useDebounce(localBounce, 800);
  const debouncedAssets = useDebounce(localAssets, 800);

  useEffect(() => {
    onSave({ ...project, title: debouncedTitle, collaborators: debouncedCollabs, owner: debouncedOwner, notes: debouncedNotes, bounce_link: debouncedBounce, assets_link: debouncedAssets });
  }, [debouncedTitle, debouncedCollabs, debouncedOwner, debouncedNotes, debouncedBounce, debouncedAssets]);
  ...
  return (
    <div style={{ background: "rgba(255,255,255,0.65)", border: `1px solid #ddd0b8`, borderLeft: `4px solid ${currentStage.color}`, borderRadius: 14, marginBottom: 12, overflow: "hidden", backdropFilter: "blur(6px)", boxShadow: expanded ? "0 4px 24px rgba(30,16,8,0.12)" : "0 2px 8px rgba(30,16,8,0.07)", transition: "box-shadow 0.2s" }}>
      ...
      <div style={{ padding: "16px 18px 0", display: "flex", alignItems: "flex-start", gap: 12 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6, flexShrink: 0 }}>
          <div style={{ background: `${currentStage.color}18`, border: `1px solid ${currentStage.color}44`, color: currentStage.color, borderRadius: 20, padding: "4px 12px", fontSize: 10, fontFamily: "'DM Mono', monospace", fontWeight: 600, letterSpacing: 1 }}>{currentStage.short}</div>
          {nextDeadline && (
            <div style={{ display: "flex", alignItems: "center", gap: 5, background: `${currentStage.color}0d`, border: `1px solid ${currentStage.color}22`, borderRadius: 6, padding: "4px 10px" }}>
              <span style={{ fontSize: 10 }}>📅</span>
              <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 10, color: "#7a5a38" }}>
                {nextDeadline.stage.short} <strong style={{ color: currentStage.color }}>{formatDate(nextDeadline.date)}</strong>
              </span>
            </div>
          )}
          {project.bounce_link && (
            <a href={project.bounce_link} target="_blank" rel="noopener noreferrer"
              ...
              onMouseEnter={e => e.currentTarget.style.background = "rgba(122,171,152,0.3)"}
              onMouseLeave={e => e.currentTarget.style.background = "rgba(122,171,152,0.15)"}>
              <span style={{ fontSize: 10 }}>🎵</span>
              <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 10, color: "#7aab98", fontWeight: 500, letterSpacing: 0.5 }}>LATEST BOUNCE ↗</span>
            </a>
          )}
        </div>
      </div>
      ...
      {/* Notes preview — always visible if there are notes */}
      {project.notes && !expanded && (
        <div onClick={onToggle} style={{ margin: "0 18px 14px", padding: "10px 14px", background: "rgba(255,255,255,0.5)", border: "1px solid #e8d8c0", borderRadius: 8, cursor: "pointer" }}>
      ...
      {expanded && (
        <div style={{ borderTop: "1px solid #e8d8c0", padding: "20px 18px", background: "rgba(255,255,255,0.4)" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 12, marginBottom: 14 }}>
            <Label label="Project / Song Title"><input value={localTitle} onChange={e => setLocalTitle(e.target.value)} placeholder="Enter title..." style={inputStyle} /></Label>
            <Label label="Collaborators"><input value={localCollabs} onChange={e => setLocalCollabs(e.target.value)} placeholder="Producer, featured artist..." style={inputStyle} /></Label>
            <Label label="Responsible for Next Steps"><input value={localOwner} onChange={e => setLocalOwner(e.target.value)} placeholder="Name or role..." style={inputStyle} /></Label>
          ...
          <Label label="Notes">
            <textarea value={localNotes} onChange={e => setLocalNotes(e.target.value)} placeholder="Session notes, references, links..." rows={3} style={{ ...inputStyle, resize: "vertical", minHeight: 70 }} />
          </Label>
          ...
          <div style={{ marginTop: 14, display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 12 }}>
            <Label label="🎵 Latest Bounce">
              <div style={{ display: "flex", gap: 8 }}>
                <input value={localBounce} onChange={e => setLocalBounce(e.target.value)} placeholder="Paste Dropbox link..." style={{ ...inputStyle, flex: 1, fontSize: 11, fontFamily: "'DM Mono', monospace" }} />
                {localBounce && <a href={localBounce} target="_blank" rel="noopener noreferrer" style={{ background: "#1e1008", color: "#f0e8da", borderRadius: 6, padding: "8px 12px", fontSize: 10, fontFamily: "'DM Mono', monospace", letterSpacing: 1, textDecoration: "none", whiteSpace: "nowrap", flexShrink: 0 }}>↗</a>}
              </div>
            </Label>
            <Label label="🖼️ Album Art / Assets">
              <div style={{ display: "flex", gap: 8 }}>
                <input value={localAssets} onChange={e => setLocalAssets(e.target.value)} placeholder="Paste Dropbox link..." style={{ ...inputStyle, flex: 1, fontSize: 11, fontFamily: "'DM Mono', monospace" }} />
                {localAssets && <a href={localAssets} target="_blank" rel="noopener noreferrer" style={{ background: "#1e1008", color: "#f0e8da", borderRadius: 6, padding: "8px 12px", fontSize: 10, fontFamily: "'DM Mono', monospace", letterSpacing: 1, textDecoration: "none", whiteSpace: "nowrap", flexShrink: 0 }}>↗</a>}
              </div>
            </Label>
          </div>
                ...
                return (
                  <div key={stage.id} style={{ background: isCurrent ? `${stage.color}0d` : "rgba(255,255,255,0.4)", border: `1px solid ${isCurrent ? stage.color + "33" : "#e8d8c0"}`, borderRadius: 8, padding: "8px 10px", opacity: isPast ? 0.5 : 1 }}>
                    ...
                    <input type="date" value={deadlines[stage.id] || ""} onChange={e => handleDeadline(stage.id, e.target.value)}
                      style={{ ...inputStyle, padding: "3px 4px", fontSize: 10, background: "transparent", border: "none", borderBottom: `1px solid ${isCurrent ? stage.color + "44" : "#e0d0b8"}`, borderRadius: 0, color: deadlines[stage.id] ? "#5a3a20" : "#b8a888", width: "100%" }} />
                  </div>
                );
              })}
            </div>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 16 }}>
            <button onClick={onDelete}
              style={{ background: "transparent", border: "1px solid rgba(180,80,50,0.2)", color: "rgba(180,80,50,0.4)", borderRadius: 6, padding: "6px 16px", fontSize: 11, fontFamily: "'DM Mono', monospace", letterSpacing: 1, cursor: "pointer", transition: "all 0.15s" }}
...
export default function MusicTracker() {
  ...
  const updateProject = (updated) => {
    ...
    supabase.from("projects").update({ stage: updated.stage, deadlines: updated.deadlines }).eq("id", updated.id);
  };

  const saveProject = async (updated) => {
    await supabase.from("projects").update({ title: updated.title, collaborators: updated.collaborators, owner: updated.owner, notes: updated.notes, bounce_link: updated.bounce_link, assets_link: updated.assets_link }).eq("id", updated.id);
  };
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 960, margin: "0 auto", padding: "32px 16px" }}>
        <div style={{ background: "#f0e8da", borderRadius: 20, padding: "clamp(24px, 5vw, 48px)", position: "relative", overflow: "hidden", boxShadow: "0 8px 48px rgba(0,0,0,0.4)" }}>
          ...
          <div style={{ display: "flex", gap: 10, marginBottom: 24, flexWrap: "wrap", alignItems: "center", position: "relative" }}>
            ...
            <button onClick={addProject} disabled={saving} style={{ marginLeft: "auto", background: saving ? "#c8b898" : "#1e1008", border: "none", borderRadius: 9, color: "#f0e8da", padding: "11px 22px", fontSize: 11, fontFamily: "'DM Mono', monospace", fontWeight: 500, letterSpacing: 2, cursor: saving ? "wait" : "pointer", boxShadow: "0 4px 16px rgba(30,16,8,0.25)", whiteSpace: "nowrap" }}>{saving ? "SAVING..." : "+ NEW PROJECT"}</button>
          </div>
          ...
          {loading ? (
            ...
            filtered.map(p => <ProjectCard key={p.id} project={p} onUpdate={updateProject} onSave={saveProject} onDelete={() => deleteProject(p.id)} expanded={expanded === p.id} onToggle={() => setExpanded(expanded === p.id ? null : p.id)} />)
          )}
