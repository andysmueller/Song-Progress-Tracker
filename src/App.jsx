...
// Debounce hook — saves to DB only after user stops typing for 800ms
      ...
      {STAGES.map((stage, i) => {
        ...
        return (
          <button key={stage.id}
            ...
            style={{ flex: 1, background: isPast ? "#b86c3055" : isCurrent ? "#b86c30" : "#e8d8c0", border: "none", borderRadius: 2, cursor: "pointer", outline: "none", transition: "all 0.15s" }} />
        );
      })}
    </div>
  );
...
function LinkField({ label, icon, value, onChange, placeholder }) {
  return (
    <div style={{ marginBottom: 0 }}>
      <div style={{ fontSize: 9, fontFamily: "'DM Mono', monospace", letterSpacing: 2, color: "#c8b898", textTransform: "uppercase", marginBottom: 6, display: "flex", alignItems: "center", gap: 6 }}>
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
        <input
          ...
          style={{ ...inputStyle, flex: 1, fontSize: 11, fontFamily: "'DM Mono', monospace" }}
        />
        {value && (
          <a href={value} target="_blank" rel="noopener noreferrer" style={{
            background: "#1e1008", color: "#f0e8da", borderRadius: 6,
            padding: "7px 12px", fontSize: 10, fontFamily: "'DM Mono', monospace",
            letterSpacing: 1, textDecoration: "none", whiteSpace: "nowrap",
            flexShrink: 0,
          }}>OPEN ↗</a>
        )}
      </div>
...
function ProjectCard({ project, onUpdate, onSave, onDelete, expanded, onToggle }) {
  ...
  // Local state for text fields — updates UI instantly, saves after debounce
  ...
  // Sync local state if project changes from outside (realtime)
  useEffect(() => {
  ...
  // Save when debounced values change
  useEffect(() => {
  ...
  return (
    <div style={{ background: "rgba(255,255,255,0.6)", border: `1px solid #ddd0b8`, borderLeft: `3px solid ${currentStage.color}`, borderRadius: 12, marginBottom: 10, overflow: "hidden", backdropFilter: "blur(6px)", boxShadow: expanded ? "0 4px 24px rgba(30,16,8,0.1)" : "0 2px 8px rgba(30,16,8,0.06)", transition: "box-shadow 0.2s" }}>
      ...
      <div style={{ padding: "16px 18px", cursor: "pointer", display: "flex", alignItems: "flex-start", gap: 14 }} onClick={onToggle}>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6, flexShrink: 0 }}>
          <div style={{ background: `${currentStage.color}18`, border: `1px solid ${currentStage.color}33`, color: currentStage.color, borderRadius: 20, padding: "3px 11px", fontSize: 9, fontFamily: "'DM Mono', monospace", fontWeight: 500, letterSpacing: 1 }}>{currentStage.short}</div>
          {nextDeadline && (
          ...
          {project.bounce_link && (
            <a href={project.bounce_link} target="_blank" rel="noopener noreferrer"
              ...
              style={{
                ...
                background: "rgba(122,171,152,0.12)",
                border: "1px solid rgba(122,171,152,0.3)",
                borderRadius: 6, padding: "3px 9px",
                textDecoration: "none",
                cursor: "pointer",
                transition: "background 0.15s",
              }}
              onMouseEnter={e => e.currentTarget.style.background = "rgba(122,171,152,0.25)"}
              onMouseLeave={e => e.currentTarget.style.background = "rgba(122,171,152,0.12)"}
            >
              <span style={{ fontSize: 10 }}>🎵</span>
              <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 9, color: "#7aab98", letterSpacing: 0.5, fontWeight: 500 }}>LATEST BOUNCE ↗</span>
            </a>
          )}
        </div>
      ...
      {expanded && (
        <div style={{ borderTop: "1px solid #e8d8c0", padding: "20px 18px 20px", background: "rgba(255,255,255,0.4)" }}>
          ...
          <Label label="Notes">
            <textarea value={localNotes} onChange={e => setLocalNotes(e.target.value)} placeholder="Session notes, references, links..." rows={2} style={{ ...inputStyle, resize: "vertical", minHeight: 52 }} />
          </Label>

          {/* Dropbox Links */}
          <div style={{ marginTop: 16, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <LinkField label="Latest Bounce" icon="🎵" value={localBounce} onChange={setLocalBounce} placeholder="Paste Dropbox link..." />
            <LinkField label="Album Art / Assets" icon="🖼️" value={localAssets} onChange={setLocalAssets} placeholder="Paste Dropbox link..." />
          </div>
...
function FilterBtn({ active, onClick, color, children }) {
  return (
    <button onClick={onClick} style={{ background: active ? color : "transparent", border: `1px solid ${active ? color : "#d0bfa0"}`, color: active ? "#f0e8da" : "#a08060", borderRadius: 20, padding: "5px 13px", fontSize: 9, fontFamily: "'DM Mono', monospace", fontWeight: 500, letterSpacing: 1, cursor: "pointer", whiteSpace: "nowrap", transition: "all 0.15s" }}>{children}</button>
  );
}
...
export default function MusicTracker() {
  ...
  // Instant update to local state (for stage changes etc)
  ...
  const saveProject = async (updated) => {
    await supabase.from("projects").update({
      ...
      owner: updated.owner,
      notes: updated.notes,
      bounce_link: updated.bounce_link,
      assets_link: updated.assets_link,
    }).eq("id", updated.id);
  };
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 900, margin: "0 auto", padding: "48px 20px" }}>
        <div style={{ background: "#f0e8da", borderRadius: 20, padding: "48px", position: "relative", overflow: "hidden", boxShadow: "0 8px 48px rgba(0,0,0,0.4)" }}>
          ...
          <div style={{ display: "flex", gap: 10, marginBottom: 28, flexWrap: "wrap", alignItems: "center", position: "relative" }}>
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search projects..." style={{ ...inputStyle, width: 220, fontSize: 11, fontFamily: "'DM Mono', monospace" }} />
            ...
            <button onClick={addProject} disabled={saving} style={{ marginLeft: "auto", background: saving ? "#c8b898" : "#1e1008", border: "none", borderRadius: 9, color: "#f0e8da", padding: "10px 22px", fontSize: 10, fontFamily: "'DM Mono', monospace", fontWeight: 500, letterSpacing: 2, cursor: saving ? "wait" : "pointer", boxShadow: "0 4px 16px rgba(30,16,8,0.25)", whiteSpace: "nowrap" }}>{saving ? "SAVING..." : "+ NEW PROJECT"}</button>
          </div>
          ...
          {loading ? (
            ...
            <div style={{ textAlign: "center", color: "#c8b898", padding: "60px 0", fontStyle: "italic" }}>No projects found. Add one above.</div>
          ) : (
