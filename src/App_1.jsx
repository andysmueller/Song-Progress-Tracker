...
function ProjectCard({ project, onDelete, onRefresh, expanded, onToggle, dragHandlers }) {
  ...
  return (
    <div {...dragHandlers} style={{ background: "#fff9f2", border: "1px solid #ddd0b8", borderRadius: 12, marginBottom: 10, overflow: "hidden", boxShadow: expanded ? "0 4px 20px rgba(26,15,6,0.1)" : "0 1px 4px rgba(26,15,6,0.06)", transition: "box-shadow 0.2s", cursor: "grab" }}>
      ...
      {/* Deadline + Release Date under bar */}
      <div style={{ padding: "5px 16px 10px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, flexWrap: "wrap" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
          ...
          {project.deadlines?.release && (
            <span style={{ fontFamily: "'DM Mono',monospace", fontSize: 9, color: "#7aab98", display: "flex", alignItems: "center", gap: 4 }}>
              <span style={{ width: 1, height: 10, background: "#e0d0b8", display: "inline-block" }} />
              🎯 Release <strong style={{ color: "#5a9a88" }}>{formatDate(project.deadlines.release)}</strong>
            </span>
          )}
        </div>
        {project.bounce_link && (
      ...
      {expanded && (
        <div style={{ borderTop: "1px solid #e8ddd0", padding: "18px 16px", background: "rgba(245,238,226,0.6)" }}>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            {/* Projected Release Date — prominent full-width tile */}
            <div style={{ background: "linear-gradient(135deg, rgba(122,171,152,0.12), rgba(106,154,120,0.08))", border: "1px solid rgba(122,171,152,0.4)", borderRadius: 9, padding: "12px 16px", marginBottom: 10, display: "flex", alignItems: "center", gap: 14 }}>
              <span style={{ fontSize: 18 }}>🎯</span>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 8, fontFamily: "'DM Mono',monospace", fontWeight: 600, letterSpacing: 2, color: "#7aab98", textTransform: "uppercase", marginBottom: 5 }}>Projected Release Date</div>
                <input type="date" value={local.deadlines["release"] || ""} onChange={e => setDeadline("release", e.target.value)}
                  style={{ background: "transparent", border: "none", borderBottom: "1px solid rgba(122,171,152,0.4)", borderRadius: 0, color: local.deadlines["release"] ? "#3a6a58" : "#c8b090", fontSize: 13, fontFamily: "'DM Mono',monospace", outline: "none", padding: "2px 0", fontWeight: 500 }} />
              </div>
              {local.deadlines["release"] && (
                <span style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 18, fontStyle: "italic", color: "#5a9a88", whiteSpace: "nowrap" }}>{formatDate(local.deadlines["release"])}</span>
              )}
            </div>
    ...
    <div style={{ minHeight: "100vh", background: "#1a0f06", fontFamily: "Georgia,serif" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 860, margin: "0 auto", padding: "32px 16px" }}>
        <div style={{ background: "#f2eadc", borderRadius: 20, padding: "clamp(24px,5vw,44px) clamp(20px,5vw,48px)", position: "relative", overflow: "hidden", boxShadow: "0 12px 60px rgba(0,0,0,0.45)" }}>
          ...
          {/* Header */}
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 28, paddingBottom: 24, borderBottom: "1px solid #ddd0b8", position: "relative" }}>
            <div>
              ...
              <h1 style={{ margin: 0, fontFamily: "'Cormorant Garamond',serif", fontSize: "clamp(36px,8vw,52px)", fontWeight: 300, color: "#1a0f06", lineHeight: 1, whiteSpace: "nowrap" }}>
                Music <em style={{ fontStyle: "italic", color: "#b86c30" }}>Tracker</em>
              </h1>
            </div>
            <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 10, paddingBottom: 4 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <div style={{ width: 5, height: 5, borderRadius: "50%", background: "#b86c30", boxShadow: "0 0 6px #b86c3088", animation: "pulse 2s infinite" }} />
                  <span style={{ fontFamily: "'DM Mono',monospace", fontSize: 8, color: "#c8a870", letterSpacing: 2.5, textTransform: "uppercase" }}>Live Sync</span>
                </div>
                <div style={{ width: 1, height: 12, background: "#d8c8b0" }} />
                <span style={{ fontFamily: "'DM Mono',monospace", fontSize: 8, color: "#a08060", letterSpacing: 2, textTransform: "uppercase" }}><strong style={{ color: "#6a4820", fontWeight: 600 }}>{projects.length}</strong> Projects</span>
              </div>
              <button onClick={addProject} disabled={adding} style={{ background: adding ? "#c8b898" : "#1a0f06", border: "none", borderRadius: 8, color: "#f2eadc", padding: "10px 22px", fontFamily: "'DM Mono',monospace", fontSize: 10, letterSpacing: 2.5, cursor: adding ? "wait" : "pointer", boxShadow: "0 3px 12px rgba(26,15,6,0.3)", whiteSpace: "nowrap" }}>{adding ? "ADDING..." : "+ NEW PROJECT"}</button>
            </div>
          </div>
